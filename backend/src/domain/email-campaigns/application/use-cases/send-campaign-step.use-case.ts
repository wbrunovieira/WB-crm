...
import { resolveStepContent, StepTranslationLite } from "../services/resolve-step-content";
...
export class SendCampaignStepUseCase {
  ...
  constructor(
    ...
    private readonly stepTranslations: EmailCampaignStepTranslationsRepository,
  ...
  async execute(input: Input): Promise<Either<Error, Output>> {
    ...
    // Localized versions of this step (base subject/bodyHtml = pt fallback).
    const stepTranslations = await this.stepTranslations.findByStep(step.id.toString());
    const translations: StepTranslationLite[] = stepTranslations.map((t) => ({ language: t.language, subject: t.subject, bodyHtml: t.bodyHtml }));
    ...
    for (const recipient of pending) {
      ...
      // Pick the version for this recipient's language (falls back to base/pt).
      const content = resolveStepContent({ subject: step.subject, bodyHtml: step.bodyHtml }, translations, recipient.language);
      ...
      if (!this.isValidEmail(recipient.email)) {
        ...
        const subject = this.resolver.resolve(content.subject, recipient);
      ...
      let subject = this.resolver.resolve(content.subject, recipient);
      let body = this.resolver.resolve(content.bodyHtml, recipient, input.trackingBaseUrl, send.id.toString());
