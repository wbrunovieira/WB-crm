...
export class NotifyHostBookingUseCase {
  ...
  async execute(input: NotifyHostBookingInput): Promise<Either<Error, { notified: boolean }>> {
    ...
    try {
      await this.createNotification.execute({
        ...
        summary: `${input.attendeeName} agendou uma reunião ${modeLabel} para ${when}.`,
      ...
      try {
        ...
        if (owner?.email) {
          await this.gmail.send({
            ...
            bodyHtml: this.buildEmail(input.attendeeName, when, modeLabel, input.meetLink),
          });
        }
      } catch (err) {
        this.logger.warn(
  ...
  private buildEmail(attendeeName: string, when: string, modeLabel: string, meetLink: string | null): string {
    const safeName = escapeHtml(attendeeName);
    const link = meetLink
      ? `<p><a href="${escapeHtml(meetLink)}" style="display:inline-block;background:#792990;color:#fff;padding:10px 18px;border-radius:8px;text-decoration:none">Abrir o Google Meet</a></p>`
      : "";
    return `
