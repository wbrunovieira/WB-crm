...
export class PrismaBookingLinksAdminRepository extends BookingLinksAdminRepository {
  ...
  async create(d: CreateBookingLinkData): Promise<BookingLinkRecord> {
    const r = await this.prisma.bookingLink.create({
      data: { token: d.token, ownerId: d.ownerId, bookingTypeId: d.bookingTypeId, leadId: d.leadId ?? null, contactId: d.contactId ?? null, label: d.label ?? null },
    });
    return { id: r.id, token: r.token, ownerId: r.ownerId, bookingTypeId: r.bookingTypeId, leadId: r.leadId, contactId: r.contactId, label: r.label, active: r.active, expiresAt: r.expiresAt };
  }
}
