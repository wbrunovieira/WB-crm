...
function getBucket(): string {
  return process.env.AWS_S3_ACTIVITY_PHOTOS_BUCKET ?? process.env.AWS_S3_GOTO_BUCKET ?? "wb-crm-recordings";
}

@Injectable()
export class ActivityPhotoS3Adapter extends ActivityPhotoStoragePort {
  buildKey(activityId: string, filename: string): string {
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    return `activity-photo/${activityId}/${timestamp}-${filename}`;
  }
