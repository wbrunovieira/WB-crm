...
export class PhoneMatcherService extends IPhoneMatcherService {
  ...
  // Matching is intentionally COMPANY-WIDE (not scoped to `ownerId`). The GoTo softphone
  // and WhatsApp number are single shared lines — the GoTo sync runs under one fixed
  // GOTO_DEFAULT_OWNER_ID — so a call/message must link to the lead regardless of which
  // internal user owns it (e.g. leads prospected by the Bot Prospector). The `ownerId`
  // param is kept for the interface/callers but no longer filters the query.
  async match(
    phone: string,
    ownerId: string,
  ): Promise<PhoneMatchResult | null> {
    ...
    if (contactRows[0]) {
      ...
      const contactCheck = await this.prisma.contact.findFirst({
        where: {
          ...
          OR: [
            { organizationId: null },
            { organization: { inOperationsAt: null } },
          ],
        },
        select: { id: true },
      });
    ...
    // 2. Search Lead (phone, phone2, and whatsapp)
    const leadRows = await this.prisma.$queryRaw<Array<{ id: string }>>`
      SELECT id FROM leads
      WHERE (
          regexp_replace(COALESCE(phone, ''), '[^0-9]', '', 'g') = ANY(${variations}::text[])
          OR regexp_replace(COALESCE(phone2, ''), '[^0-9]', '', 'g') = ANY(${variations}::text[])
          OR regexp_replace(COALESCE(whatsapp, ''), '[^0-9]', '', 'g') = ANY(${variations}::text[])
        )
      LIMIT 1
    `;
    ...
    if (!leadId) {
      ...
      const lcRows = await this.prisma.$queryRaw<Array<{ leadId: string }>>`
        SELECT "leadId" FROM lead_contacts
        WHERE (
            regexp_replace(COALESCE(phone, ''), '[^0-9]', '', 'g') = ANY(${variations}::text[])
            OR regexp_replace(COALESCE(whatsapp, ''), '[^0-9]', '', 'g') = ANY(${variations}::text[])
          )
