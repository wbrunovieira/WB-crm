...
export class PrismaOrganizationsRepository extends OrganizationsRepository {
  ...
  async findMany(
    ...
    return rows.map((row) => ({
      ...
      whatsappPhoneType: row.whatsappPhoneType,
      whatsappPhoneValid: row.whatsappPhoneValid,
  ...
  async findById(id: string, requesterId: string, requesterRole: string): Promise<OrganizationDetail | null> {
      ...
      include: {
        ...
        activities: {
          select: {
            ...
            createdAt: true,
            // Espelho do select do lead. A pagina da organizacao usa a mesma timeline rica, e
            // sem estes campos ela renderizava uma versao pobre: sem gravacao de ligacao, sem
            // transcricao, sem thread de e-mail, sem o contato vinculado. Era esse o "atividades
            // listadas diferentes" que o Bruno viu — dado que faltava, nao componente.
            contactId: true,
            leadContactIds: true,
    ...
    return {
      ...
      activities: row.activities.map((a) => ({
        ...
        emailCampaignSendId: a.emailCampaignSendId,
        emailCampaignId: a.emailCampaignId,
        createdAt: a.createdAt,
        // Terceiro lugar que enumera campo a campo: o select busca, o read model declara e o
        // mapper converte. Um campo esquecido AQUI some em silencio, com HTTP 200 e tudo.
