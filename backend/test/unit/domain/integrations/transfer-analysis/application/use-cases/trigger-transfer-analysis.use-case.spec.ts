...
const baseInput = () => ({
  activityId: "act-1",
  activitySubject: "Ligação transferência",
  transcript: "SDR: Bom dia, aqui é Bruno... GK: Um momento... Decisor: Alô, Carlos aqui.",
...
describe("TriggerTransferAnalysisUseCase", () => {
  ...
  beforeEach(() => {
    ...
    sut = new TriggerTransferAnalysisUseCase(gkRepo, callRepo, agentPort);
  });
  ...
  it("passes transcript and lead data to agent", async () => {
  ...
  it("reuses existing pending GK analysis — does not duplicate", async () => {
    const existing = GatekeeperAnalysis.create({
      activityId: "act-1", ownerId: "user-1", status: "pending", jobId: "gk-job-old",
    });
    await gkRepo.save(existing);
    ...
    expect(gkRepo.items).toHaveLength(1);
    if (result.isRight()) {
      expect(result.value.gkAnalysisId).toBe(existing.id.toString());
    }
  });

  it("reuses existing pending SPICED analysis — does not duplicate", async () => {
    const existing = CallAnalysis.create({
      activityId: "act-1", ownerId: "user-1", status: "pending", jobId: "spiced-job-old",
    });
    await callRepo.save(existing);
    ...
    expect(callRepo.items).toHaveLength(1);
    if (result.isRight()) {
      expect(result.value.spicedAnalysisId).toBe(existing.id.toString());
    }
  });
