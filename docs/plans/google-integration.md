...
| 5f | Meet: cron `check-rsvp` — atualiza status de confirmação de presença dos convidados a cada 5 min | ✅ Concluída |
...
| **Envio em lote** | Selecionar N leads e enviar o mesmo e-mail para todos com 1 clique | 🔴 Não iniciado |
| **Confirmação de leitura** | Pixel de rastreamento para saber quando o destinatário abriu o e-mail | 🔴 Não iniciado |

---

## Fase 7 — Gmail: envio em lote com confirmação de leitura

> **Ainda não planejado em detalhe** — ver seção abaixo para esboço inicial.

### Objetivo

Permitir ao SDR selecionar múltiplos leads/contatos na lista e disparar o mesmo e-mail (ou e-mail personalizado com variáveis dinâmicas) para todos, com rastreamento de abertura.

### Conceitos

**Envio em lote:**
- Seleciona N leads na `LeadsTable` (checkbox já existe)
- Clica "Enviar e-mail em lote"
- Escolhe template ou escreve o e-mail
- Variáveis dinâmicas são resolvidas individualmente por destinatário (`{{businessName}}`, `{{contactName}}`, etc.)
- Cada envio gera uma `Activity` do tipo `email` no lead correspondente
- Envios são feitos em sequência com delay de 1-2s entre eles para evitar limite de rate da Gmail API (250 quota units/s)

**Confirmação de leitura (pixel de rastreamento):**
- Ao enviar, injeta um `<img>` invisível 1×1 px no corpo HTML do e-mail:
  ```html
  <img src="https://crm.wbdigitalsolutions.com/api/email-track/[trackingId]" width="1" height="1" style="display:none" />
  ```
- Quando o destinatário abre o e-mail, o cliente de e-mail carrega a imagem → CRM registra abertura
- A rota `/api/email-track/[trackingId]` retorna a imagem (1×1 GIF transparente) e atualiza `Activity.emailOpenedAt` no banco

### Modelo de dados adicional necessário

```prisma
// Em Activity — campos a adicionar
emailTrackingId  String?   @unique  // UUID para o pixel de rastreamento
emailOpenedAt    DateTime?           // quando o destinatário abriu
emailOpenCount   Int       @default(0) // total de aberturas (múltiplos opens)
```

### Backend (TDD)

- `src/actions/gmail-bulk.ts` — `bulkSendEmail(leadIds, templateId?, subject, body)`:
  - Itera leads, resolve variáveis por destinatário, chama `sendGmailMessage()` com trackingId injetado
  - Retorna `{ sent: number, failed: number, errors: string[] }`
- `src/app/api/email-track/[trackingId]/route.ts` — endpoint público:
  - Retorna imagem GIF 1×1 transparente (`image/gif`)
  - Atualiza `Activity` com `emailOpenedAt` e incrementa `emailOpenCount`
  - Não loga erros se trackingId não encontrado (silencioso)
- `src/lib/google/gmail-bulk.ts` — orquestra envios com rate limiting (delay entre envios)

### Frontend

- `LeadsTable`: botão "Enviar e-mail" na bulk action bar (ao lado de "Arquivar" e "Aplicar Cadência")
- `BulkEmailModal`: modal com seletor de template + editor rico + preview com variáveis resolvidas para o 1º destinatário
- `Activity card`: ícone de olho + "Aberto em [data]" quando `emailOpenedAt` preenchido

### Limitações conhecidas

| Limitação | Detalhe |
|---|---|
| Gmail bloqueia pixels | Gmail (cliente web) bloqueia imagens externas por padrão — usuário precisa clicar "Exibir imagens". Outros clientes (Outlook, Apple Mail) carregam automaticamente. |
| Apple MPP | A partir de iOS 15, o Mail Privacy Protection pré-carrega imagens — pode gerar falsos positivos de abertura. |
| Rate limit Gmail API | 250 quota units/s. Envios em lote acima de ~100 destinatários precisam de delay entre envios ou usar Gmail Batch API. |
| Não é marketing em massa | Gmail detecta spam se muitos e-mails idênticos forem enviados. Usar variáveis dinâmicas e limitar a lotes de até 50 destinatários/dia pela conta conectada. |

### Critérios de aceite

- [ ] Selecionar 5 leads → enviar e-mail com variáveis resolvidas por lead → 5 Activities criadas
- [ ] Abrir e-mail no Outlook → Activity mostra "Aberto em [data]"
- [ ] Destinatário com `emailOpenedAt` null → Activity mostra "Não aberto"
- [ ] Envio com template substitui `{{businessName}}` pelo nome correto de cada lead
- [ ] Falha em 1 envio não cancela os demais (continue on error)
