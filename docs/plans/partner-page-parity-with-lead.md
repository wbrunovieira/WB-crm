...
**Status:** ✅ CONCLUÍDO — Fases 1, 2, 3 (a+b) e 4 (a–g) todas entregues (última atualização 2026-07-12). Resta apenas eventual limpeza de produto (aparar campos niche do ICP, revisar vocabulário de cadência p/ partner).
...
- **Fase 4 — Grupo C (portar recursos escolhidos):** **Decisão do Bruno (2026-07-11):** portar 7 recursos do grupo C (fora Hierarquia e os de prospecção — Google Places/Meta-Google Ads/Deep Research). Insight estrutural: `Organization` já tem o precedente (junctions `Organization*`, colunas `primaryCNAEId`/`internationalActivity`/`languages`) → portar = replicar o padrão em `Partner*`. Migração só no backend, aditiva. TDD obrigatório + review por sub-fase + build + push (sem deploy por fase). Sub-fases por valor×esforço:
  ...
  - **4b — Idiomas:** ✅ CONCLUÍDA — coluna `Partner.languages String?` (JSON `[{code,isPrimary}]`, migration `20260711000004`) em toda a stack (create+update); reuso dos componentes `shared/` `LanguageSelector` (form) e `LanguageBadges` (detalhe). Convenção "`null` limpa" (vazio no form → `null` → apaga; omitido → preserva) — melhoria sobre o Lead, que ainda tem a limitação vazio→undefined (dívida transversal). Unit + e2e (set/preserve/clear). Senior review: aprovado.
  - **4c — Verify e-mail/telefone:** ✅ CONCLUÍDA — 8 colunas de verificação no `Partner` (email verified/at/status/reason; phone valid/type; whatsapp valid/type; migration `20260711000005`); use-cases `VerifyPartnerEmailUseCase`/`VerifyPartnerPhonesUseCase` (domínio phone) reusando `EmailVerifierPort`/`PhoneValidatorPort` **com ownership check** (owner/admin — o Contact não tinha, o Lead tem); persistência por métodos dedicados (`saveEmailVerification`/`savePhoneVerification`) sem tocar entity/mapper (um PATCH comum não zera); rotas `POST /phone/verify/partner/:id` e `/:id/email` (403/404). Frontend: `PartnerEmailVerifyButton` + `PartnerPhoneVerifyButton` inline nos campos email/telefone. Unit + e2e (incl. ownership 403). Senior review: aprovado. **Dívidas transversais registradas:** (a) o check owner/admin espelha o Lead — nenhum dos dois consulta `SharedEntity`, então um usuário com acesso compartilhado vê o botão e leva 403 ao clicar (uniformizar SharedEntity no verify de Lead+Contact+Partner é tarefa à parte); (b) o verify de Contact ainda não tem ownership check.
  - **4d — Setor / CNAEs:** ✅ CONCLUÍDA — `Partner.primaryCNAEId` (FK SetNull) + `Partner.internationalActivity` + junctions `PartnerSector`/`PartnerSecondaryCNAE` (migration `20260711000006`), espelhando `Organization*`. Domínios agnósticos `sectors`/`cnae` ganharam use-cases + rotas `/sectors/partners/:id` e `/cnaes/partners/:id` (sector owner-scoped; CNAE global). Frontend: `PartnerSectorSection` + hooks; `SecondaryCNAEsManager` refatorado (ternário lead/org → mapa `ENTITY_PATH_SEGMENT` com `partner`, sem regredir lead/org); `PartnerForm` reusa `CNAEAutocomplete` + atividade internacional; bloco CNAE + Setores na página. Unit (sectors/cnae/partner) + e2e (fluxo completo primary/secondary/sector). Senior review: aprovado — migration×schema confirmados (incl. a diferença de `@map` camelCase vs snake_case entre os dois junctions). Dívidas pré-existentes herdadas do Organization (não corrigidas aqui): `PartnerSectorSection` renderiza campos de market-intelligence que o endpoint de lista não devolve (idêntico ao org); `primaryCNAEId` inválido estoura FK (500) em vez de `Either`. Complementa o `Partner.industry` (string livre, mantido).
  - **4e — Tech Profile:** ✅ CONCLUÍDA — 7 junctions `PartnerLanguage/Framework/Hosting/Database/ERP/CRM/Ecommerce` (migration `20260711000007`). Diferente de CNAE/Sector, o Tech Profile é **um domínio genérico único** (`tech-profile`) já parametrizado para lead+org — então só **estendi com a 3ª entidade** (port +3 métodos, repo `PARTNER_JUNCTION`/`UNIQUE_PARTNER_KEY` maps + 3 métodos reusando `getEntityItems`, +3 use-cases, +3 rotas `/partners/:id/tech-profile`). Catálogo admin (`/admin/tech-options/:type`) é global — 0 mudança. Frontend: `TechProfileBadge`/`AddTechProfileModal` refatorados (ternário lead/org → mapa `ENTITY_PATH_SEGMENT` com `partner`, sem regredir lead/org); `PartnerTechProfileSection` copiado do **Lead** (shape `TechItem[]` correto; o do Org tem bug de wrapper — dívida à parte). Unit + e2e cobrindo os **7 tipos** end-to-end (add/get/remove + idempotência + 422). Senior review: aprovado.
  - **4f — ICP / Qualificação:** ✅ CONCLUÍDA — `PartnerICP` (17 campos de qualificação, espelha `OrganizationICP`; migration `20260712000001`). Como tech-profile, o ICP é **domínio centralizado** — estendi o domínio `icp` com a 3ª entidade (port +4, prisma repo +4 reusando `toLinkData`/`toDbLinkData`, +4 use-cases com validação de ownership do **ICP** como lead/org, +4 rotas `/icps/partners/:id`). Frontend: hooks (`ICPPartnerLink`, `usePartnerICPs`, link/update/unlink) + `PartnerICPSection` (3ª cópia via sed do OrganizationICPSection). Unit + e2e (link/get/update/unlink + 403 de ICP alheio em POST/PATCH/DELETE + serialização de array). Senior review: aprovado. Os 17 campos (incl. niche de plataforma de curso) foram copiados integralmente por decisão de paridade total — aparar campos que não fizerem sentido pro partner fica como ajuste de produto futuro.
  - **4g — Cadência:** ✅ CONCLUÍDA — a única sem precedente Organization (feita à mão). **Decisões do Bruno (2026-07-12):** (1) "igual ao lead" (mantém vocabulário de prospecção); (2) **reusar o template `Cadence`** (só a instância é nova); (3) **sem filtro ICP** no "disponíveis". Schema: 2 models de instância `PartnerCadence`/`PartnerCadenceActivity` (espelham LeadCadence/LeadCadenceActivity) + 5 relações inversas; migration `20260712000002`. Backend: estende o domínio `cadences` com `applyToPartner` (gera Activities com `partnerId`, sem cron), get-detail, pause/resume/cancel/complete, `getAvailableCadencesForPartner` (sem join de ICP), `registerPartnerReply`; +8 use-cases, +9 rotas `/cadences/partner...`. Frontend: `PartnerCadenceSection` + `PartnerApplyCadenceModal` (sed do lead, `isConverted` removido, empty-state corrigido p/ refletir "sem ICP"). Unit + e2e (fluxo completo apply→2 activities→available exclui aplicada→pause/resume→reply cancela+skip). Senior review: aprovado.

**🎉 Fase 4 COMPLETA (7/7 sub-fases).** Todo o grupo C portado para o Partner.
