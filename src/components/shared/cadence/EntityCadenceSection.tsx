...
import { CompleteWithReasonModal } from "@/components/leads/CompleteWithReasonModal";
import {
...
interface EntityCadenceActivity {
  id: string;
...
// Structural shape shared by LeadCadence / PartnerCadence — the two records
// differ only in their entity id field (leadId / partnerId), which the render
// never touches, so this base covers everything the JSX reads.
export interface EntityCadence {
  id: string;
  cadenceId: string;
  status: string;
  startDate: string;
  cadence: { name: string; slug: string; durationDays: number; icp?: { id: string; name: string } | null };
  activities: EntityCadenceActivity[];
  progress: number;
  completedSteps: number;
  totalSteps: number;
}

// Entity-specific pt-BR copy that differs between lead / partner.
export interface EntityCadenceLabels {
  // Empty-state line, e.g. "Nenhuma cadência aplicada a este lead."
  emptyState: string;
  // Resume confirmation message (lead mentions date adjustment, partner does not).
  resumeMessage: string;
}

// Minimal mutation surface used for pause/resume/cancel. The react-query
// useMutation result is structurally assignable to this.
interface CadenceMutation<TArg> {
  mutateAsync: (variables: TArg) => Promise<unknown>;
}

interface EntityCadenceSectionProps<TArg extends Record<string, string>> {
  entityId: string;
  // Path segment for the cadence endpoints: "lead" | "partner"
  // (drives /cadences/{entity}/:id and /cadences/{entity}-cadences/:id/complete).
  entity: string;
  isConverted?: boolean;
  labels: EntityCadenceLabels;
  hooks: {
    usePause: () => CadenceMutation<TArg>;
    useResume: () => CadenceMutation<TArg>;
    useCancel: () => CadenceMutation<TArg>;
  };
  // Builds the pause/resume/cancel mutation payload for a given cadence id,
  // e.g. (id) => ({ leadCadenceId: id, leadId }) / ({ partnerCadenceId: id, partnerId }).
  mutationArg: (cadenceId: string) => TArg;
  renderModal: (opts: { onClose: () => void; onSuccess: () => void }) => React.ReactNode;
}

export function EntityCadenceSection<TArg extends Record<string, string>>({
  entityId,
  entity,
  isConverted = false,
  labels,
  ...
  mutationArg,
  renderModal,
}: EntityCadenceSectionProps<TArg>) {
  const router = useRouter();
  const { data: session } = useSession();
  const token = session?.user?.accessToken ?? "";
  const pauseMutation = hooks.usePause();
  const resumeMutation = hooks.useResume();
  const cancelMutation = hooks.useCancel();
  const [cadences, setCadences] = useState<EntityCadence[]>([]);
  ...
  const fetchCadences = () => apiFetch<EntityCadence[]>(`/cadences/${entity}/${entityId}`, token);

  useEffect(() => {
    async function loadCadences() {
      try {
        const data = await apiFetch<EntityCadence[]>(`/cadences/${entity}/${entityId}`, token);
        setCadences(data);
      } catch {
  ...
  const handlePause = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      await pauseMutation.mutateAsync(mutationArg(id));
  ...
  const handleResume = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      await resumeMutation.mutateAsync(mutationArg(id));
      const data = await fetchCadences();
      setCadences(data);
      router.refresh();
    } catch (error) {
  ...
  const handleCancel = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      await cancelMutation.mutateAsync(mutationArg(id));
      const data = await fetchCadences();
      setCadences(data);
      router.refresh();
    } catch (error) {
  ...
  const handleCompleteConfirm = async (reason: string | undefined) => {
    ...
    try {
      await apiFetch(`/cadences/${entity}-cadences/${completeTarget}/complete`, token, { method: "PATCH", body: JSON.stringify({ disqualificationReason: reason }) });
