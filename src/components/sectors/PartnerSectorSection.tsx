...
import {
  usePartnerSectors,
  useSectors,
  useLinkPartnerToSector,
  useUnlinkPartnerFromSector,
} from "@/hooks/sectors/use-sectors";
import { toast } from "sonner";
import { useConfirmDialog, ConfirmDialog } from "@/components/shared/ConfirmDialog";

interface PartnerSectorSectionProps {
  partnerId: string;
}
...
export function PartnerSectorSection({ partnerId }: PartnerSectorSectionProps) {
  ...
  const { data: linked = [] } = usePartnerSectors(partnerId);
  const { data: allSectors = [] } = useSectors();
  const linkMutation = useLinkPartnerToSector();
  const unlinkMutation = useUnlinkPartnerFromSector();
  ...
  const handleLink = async () => {
    ...
    try {
      await linkMutation.mutateAsync({ partnerId, sectorId: selectedSectorId });
  ...
  const handleUnlink = async (sectorId: string, sectorName: string) => {
    const confirmed = await confirm({
      ...
      message: `Remover "${sectorName}" deste parceiro?`,
    ...
    try {
      await unlinkMutation.mutateAsync({ partnerId, sectorId });
