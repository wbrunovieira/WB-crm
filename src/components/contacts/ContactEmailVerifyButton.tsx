...
export function ContactEmailVerifyButton({
  email,
  contactId,
  verified,
}: ContactEmailVerifyButtonProps) {
  ...
  if (uiStatus === "idle" && hasVerified) {
    const badgeStatus = statusToUiStatus(verified!.status!, verified!.valid!);
    return (
      <span className="inline-flex items-center gap-1.5 flex-wrap">
        <BadgeForStatus status={badgeStatus} reason={verified!.reason ?? ""} verifiedAt={verified!.at ?? undefined} />
        <button
  ...
  async function handleVerify() {
    ...
    try {
      const result = await apiFetch<{ ok: boolean; contactId: string; email: string; valid: boolean; status: string; reason: string }>(
        `/phone/verify/contact/${contactId}/email`,
        token,
        { method: "POST" },
      );
