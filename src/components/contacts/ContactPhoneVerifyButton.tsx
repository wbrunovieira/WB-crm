...
export function ContactPhoneVerifyButton({
  contactId,
  phone,
  whatsapp,
  existing,
}: ContactPhoneVerifyButtonProps) {
  ...
  const hasAnyPhone = !!(phone || whatsapp);
  ...
  async function handleVerify() {
    ...
    try {
      const result = await apiFetch<VerifyResponse>(
        `/phone/verify/contact/${contactId}`,
        token,
        { method: "POST" },
      );
    ...
    return (
      <span className="inline-flex items-center gap-1.5 flex-wrap">
        {existing!.phoneValid !== undefined && existing!.phoneValid !== null && phone && (
          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${existing!.phoneValid ? "bg-green-100 text-green-700" : "bg-red-50 text-red-500"}`}>
            {existing!.phoneValid ? <CheckIcon className="h-3 w-3" /> : <XIcon className="h-3 w-3" />}
            Tel {existing!.phoneValid ? "válido" : "inválido"}
          </span>
        )}
        {existing!.whatsappPhoneValid !== undefined && existing!.whatsappPhoneValid !== null && whatsapp && (
          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${existing!.whatsappPhoneValid ? "bg-green-100 text-green-700" : "bg-red-50 text-red-500"}`}>
            {existing!.whatsappPhoneValid ? <CheckIcon className="h-3 w-3" /> : <XIcon className="h-3 w-3" />}
            WA {existing!.whatsappPhoneValid ? "válido" : "inválido"}
          </span>
        )}
        <button
