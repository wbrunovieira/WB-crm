...
interface PartnerCadenceActivity {
  id: string;
...
async function fetchPartnerCadences(partnerId: string, token: string): Promise<PartnerCadence[]> {
  return apiFetch<PartnerCadence[]>(`/cadences/partner/${partnerId}`, token);
}
...
export function PartnerCadenceSection({ partnerId }: PartnerCadenceSectionProps) {
  ...
  const pauseMutation = usePausePartnerCadence();
  const resumeMutation = useResumePartnerCadence();
  const cancelMutation = useCancelPartnerCadence();
  const [cadences, setCadences] = useState<PartnerCadence[]>([]);
  ...
  useEffect(() => {
    async function loadCadences() {
      try {
        const data = await apiFetch<PartnerCadence[]>(`/cadences/partner/${partnerId}`, token);
        setCadences(data);
      } catch {
  ...
  const handlePause = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      await pauseMutation.mutateAsync({ partnerCadenceId: id, partnerId });
      const data = await fetchPartnerCadences(partnerId, token);
      setCadences(data);
      router.refresh();
    } catch (error) {
  ...
  const handleResume = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      await resumeMutation.mutateAsync({ partnerCadenceId: id, partnerId });
      const data = await fetchPartnerCadences(partnerId, token);
      setCadences(data);
      router.refresh();
    } catch (error) {
  ...
  const handleCancel = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      await cancelMutation.mutateAsync({ partnerCadenceId: id, partnerId });
      const data = await fetchPartnerCadences(partnerId, token);
      setCadences(data);
      router.refresh();
    } catch (error) {
  ...
  const handleCompleteConfirm = async (reason: string | undefined) => {
    ...
    try {
      await apiFetch(`/cadences/partner-cadences/${completeTarget}/complete`, token, { method: "PATCH", body: JSON.stringify({ disqualificationReason: reason }) });
        ...
        {cadences.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="mt-4 text-gray-500">
              Nenhuma cadência aplicada a este parceiro.
            </p>
            <button
              onClick={() => setShowModal(true)}
