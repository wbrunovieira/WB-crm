"use client";

import { useState, useEffect } from "react";
import { useSession } from "next-auth/react";
import { apiFetch } from "@/lib/api-client";
import { Database, Plus } from "lucide-react";
import { AddTechProfileModal } from "@/components/leads/AddTechProfileModal";
import { TechProfileBadge } from "@/components/shared/TechProfileBadge";

interface PartnerTechProfileSectionProps {
  partnerId: string;
}

interface TechItem {
  id: string;
  name: string;
  slug: string;
  color?: string | null;
  icon?: string | null;
}

interface TechProfile {
  languages: TechItem[];
  frameworks: TechItem[];
  hosting: TechItem[];
  databases: TechItem[];
  erps: TechItem[];
  crms: TechItem[];
  ecommerce: TechItem[];
}

export function PartnerTechProfileSection({ partnerId }: PartnerTechProfileSectionProps) {
  const { data: session } = useSession();
  const token = session?.user?.accessToken ?? "";
  const [techProfile, setTechProfile] = useState<TechProfile | null>(null);
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);

  useEffect(() => {
    if (!token) return;
    const loadTechProfile = async () => {
      try {
        const data = await apiFetch<TechProfile>(`/partners/${partnerId}/tech-profile`, token);
        setTechProfile(data);
      } catch (error) {
        console.error("Erro ao carregar tech profile:", error);
      } finally {
        setLoading(false);
      }
    };
    loadTechProfile();
  }, [partnerId, token]);

  const refreshTechProfile = async () => {
    try {
      const data = await apiFetch<TechProfile>(`/partners/${partnerId}/tech-profile`, token);
      setTechProfile(data);
    } catch (error) {
      console.error("Erro ao carregar tech profile:", error);
    }
  };

  const hasAnyTech = techProfile && (
    (techProfile.languages?.length ?? 0) > 0 ||
    (techProfile.frameworks?.length ?? 0) > 0 ||
    (techProfile.hosting?.length ?? 0) > 0 ||
    (techProfile.databases?.length ?? 0) > 0 ||
    (techProfile.erps?.length ?? 0) > 0 ||
    (techProfile.crms?.length ?? 0) > 0 ||
    (techProfile.ecommerce?.length ?? 0) > 0
  );

  if (loading) {
    return (
      <div className="mt-6 rounded-xl bg-white p-6 shadow-md">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Database className="h-6 w-6 text-gray-500" />
            <h2 className="text-xl font-bold text-gray-900">Perfil Tecnológico</h2>
          </div>
        </div>
        <p className="text-sm text-gray-500">Carregando...</p>
      </div>
    );
  }

  return (
    <>
      <div className="mt-6 rounded-xl bg-white p-6 shadow-md hover:shadow-lg transition-shadow duration-200">
        <div className="flex items-center justify-between mb-5 pb-3 border-b-2 border-gray-100">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🔧</span>
            <h2 className="text-xl font-bold text-gray-900">Perfil Tecnológico</h2>
            <span className="text-sm text-gray-500">(Stack Atual do Parceiro)</span>
          </div>
          <button
            onClick={() => setIsModalOpen(true)}
            className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-purple-700"
          >
            <Plus className="h-4 w-4" />
            Adicionar
          </button>
        </div>

        {!hasAnyTech ? (
          <p className="text-sm text-gray-500">
            Nenhuma informação tecnológica cadastrada para este parceiro.
          </p>
        ) : (
          <div className="space-y-4">
            {(techProfile.languages?.length ?? 0) > 0 && (
              <TechProfileBadge
                title="Linguagens"
                items={techProfile.languages}
                entityId={partnerId}
                entityType="partner"
                profileType="languages"
                onUpdate={refreshTechProfile}
              />
            )}

            {(techProfile.frameworks?.length ?? 0) > 0 && (
              <TechProfileBadge
                title="Frameworks"
                items={techProfile.frameworks}
                entityId={partnerId}
                entityType="partner"
                profileType="frameworks"
                onUpdate={refreshTechProfile}
              />
            )}

            {(techProfile.hosting?.length ?? 0) > 0 && (
              <TechProfileBadge
                title="Hospedagem"
                items={techProfile.hosting}
                entityId={partnerId}
                entityType="partner"
                profileType="hosting"
                onUpdate={refreshTechProfile}
              />
            )}

            {(techProfile.databases?.length ?? 0) > 0 && (
              <TechProfileBadge
                title="Bancos de Dados"
                items={techProfile.databases}
                entityId={partnerId}
                entityType="partner"
                profileType="databases"
                onUpdate={refreshTechProfile}
              />
            )}

            {(techProfile.erps?.length ?? 0) > 0 && (
              <TechProfileBadge
                title="ERPs"
                items={techProfile.erps}
                entityId={partnerId}
                entityType="partner"
                profileType="erps"
                onUpdate={refreshTechProfile}
              />
            )}

            {(techProfile.crms?.length ?? 0) > 0 && (
              <TechProfileBadge
                title="CRMs"
                items={techProfile.crms}
                entityId={partnerId}
                entityType="partner"
                profileType="crms"
                onUpdate={refreshTechProfile}
              />
            )}

            {(techProfile.ecommerce?.length ?? 0) > 0 && (
              <TechProfileBadge
                title="E-commerce"
                items={techProfile.ecommerce}
                entityId={partnerId}
                entityType="partner"
                profileType="ecommerces"
                onUpdate={refreshTechProfile}
              />
            )}
          </div>
        )}
      </div>

      <AddTechProfileModal
        entityId={partnerId}
        entityType="partner"
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        onSuccess={refreshTechProfile}
      />
    </>
  );
}
