...
export function PartnerEmailVerifyButton({
  email,
  partnerId,
  verified,
}: PartnerEmailVerifyButtonProps) {
  ...
  async function handleVerify() {
    ...
    try {
      const result = await apiFetch<{ ok: boolean; partnerId: string; email: string; valid: boolean; status: string; reason: string }>(
        `/phone/verify/partner/${partnerId}/email`,
        token,
        { method: "POST" },
      );
