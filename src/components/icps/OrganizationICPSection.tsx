...
import { getOrganizationICPs, linkOrganizationToICP, unlinkOrganizationFromICP, updateOrganizationICP } from "@/actions/icp-links";
...
export function OrganizationICPSection({ organizationId }: OrganizationICPSectionProps) {
  ...
  const [linkedICPs, setLinkedICPs] = useState<OrganizationICP[]>([]);
  ...
  const loadData = useCallback(async () => {
    try {
      const [links, icps] = await Promise.all([
        getOrganizationICPs(organizationId),
  ...
  const handleUnlink = async (icpId: string) => {
    ...
    setUnlinking(icpId);
    try {
      await unlinkOrganizationFromICP(organizationId, icpId);
      await loadData();
      router.refresh();
    } catch (err) {
  ...
  const handleExpand = (link: OrganizationICP) => {
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow">
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
        ...
        {unlinkedICPs.length > 0 && !showForm && (
          <button
      ...
      {linkedICPs.length === 0 ? (
        <p className="text-base text-gray-500">
          Nenhum ICP vinculado a esta organização.
                ...
                <div className="border-t border-gray-100 bg-gray-50 p-5">
                  ...
                  <div className="mb-5 grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Observações
                        <Tooltip tip="Notas adicionais sobre a organização" />
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="mb-5 grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Status de Fit
                        <Tooltip tip="Quão bem a organização se encaixa no perfil ideal" />
                      </label>
                      <select
                  ...
                  <div className="mb-5">
                    <label className="mb-2 flex items-center text-sm font-semibold text-gray-700">
                      Momento do Negócio
                      <Tooltip tip="Em qual fase do negócio a organização está" />
                    </label>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mb-5">
                    <label className="mb-2 flex items-center text-sm font-semibold text-gray-700">
                      Plataformas Atuais
                      <Tooltip tip="Quais plataformas a organização usa atualmente" />
                    </label>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mb-5 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Fragmentação (0-10)
                        <Tooltip tip="Quão fragmentada está a operação (0=unificada, 10=muito fragmentada)" />
                      </label>
                      <input
                        type="number"
                        min="0"
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Dor Principal
                        <Tooltip tip="Principal dor/problema declarado" />
                      </label>
                      <select
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Desejo Estratégico
                        <Tooltip tip="O que a organização deseja alcançar" />
                      </label>
                      <select
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Complexidade (1-5)
                        <Tooltip tip="Complexidade técnica percebida (1=simples, 5=muito complexa)" />
                      </label>
                      <input
                        type="number"
                        min="1"
                  ...
                  <div className="mb-5 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="col-span-2">
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Gatilho de Compra
                        <Tooltip tip="O que motivou a buscar uma solução agora" />
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Tempo Decisão
                        <Tooltip tip="Estimativa de quando vai tomar a decisão" />
                      </label>
                      <select
