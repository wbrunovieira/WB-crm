...
import { useICPs, type ICPLinkData } from "@/hooks/icps/use-icps";
import type {
  ICPFitStatus,
  RealDecisionMaker,
  BusinessMoment,
...
// Entity-specific pt-BR copy (the only user-facing text that differs between
// lead / partner / organization). The visual chrome is standardized on the
// lead card look for all three.
export interface EntityICPLabels {
  emptyState: string;
  notesTip: string;
  fitStatusTip: string;
  businessMomentTip: string;
  platformsTip: string;
  strategicDesireTip: string;
  painTip: string;
  fragmentationTip: string;
  purchaseTriggerTip: string;
  decisionTimeTip: string;
  complexityTip: string;
}

// Structural shape shared by ICPLeadLink / ICPPartnerLink / ICPOrgLink — the
// three entity link types differ only in their extra id field, so the render
// (which touches only icpId, icp and the qualification fields) accepts this base.
export type EntityICPLink = ICPLinkData & {
  icpId: string;
  icp: { id: string; name: string; slug: string; status: string };
};

// Minimal mutation surface the component uses (react-query useMutation result
// is structurally assignable to this).
interface ICPMutation<TVars> {
  mutateAsync: (variables: TVars) => Promise<unknown>;
  isPending: boolean;
}

interface EntityICPSectionProps<TId extends Record<string, string>> {
  entityId: string;
  isConverted?: boolean;
  idParam: TId;
  labels: EntityICPLabels;
  hooks: {
    useLinks: (id: string) => { data?: EntityICPLink[]; isLoading: boolean };
    useLink: () => ICPMutation<TId & { icpId: string } & ICPLinkData>;
    useUpdate: () => ICPMutation<TId & { icpId: string } & ICPLinkData>;
    useUnlink: () => ICPMutation<TId & { icpId: string }>;
  };
}

export function EntityICPSection<TId extends Record<string, string>>({
  entityId,
  isConverted = false,
  idParam,
  labels,
  hooks,
}: EntityICPSectionProps<TId>) {
  const { data: linkedICPs = [], isLoading: loading } = hooks.useLinks(entityId);
  const { data: availableICPs = [] } = useICPs("active");
  const linkMutation = hooks.useLink();
  const updateMutation = hooks.useUpdate();
  const unlinkMutation = hooks.useUnlink();
  ...
  const handleLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } as TId & { icpId: string } & ICPLinkData);
      setShowForm(false);
      setSelectedICP("");
  ...
  const handleUnlink = async (icpId: string) => {
    ...
    setUnlinking(icpId);
    try {
      await unlinkMutation.mutateAsync({ ...idParam, icpId } as TId & { icpId: string });
    } catch (err) {
  ...
  const handleExpand = (link: EntityICPLink) => {
                ...
                <div className="border-t border-gray-100 bg-gray-50 p-5">
                  ...
                  <div className="mb-5 grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Status de Fit
                        <Tooltip tip={labels.fitStatusTip} />
                      </label>
                      <select
                  ...
                  <div className="mb-5">
                    <label className="mb-2 flex items-center text-sm font-semibold text-gray-700">
                      Momento do Negócio
                      <Tooltip tip={labels.businessMomentTip} />
                    </label>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mb-5">
                    <label className="mb-2 flex items-center text-sm font-semibold text-gray-700">
                      Plataformas Atuais
                      <Tooltip tip={labels.platformsTip} />
                    </label>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mb-5 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Fragmentação (0-10)
                        <Tooltip tip={labels.fragmentationTip} />
                      </label>
                      <input
                        type="number"
                        min="0"
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Desejo Estratégico
                        <Tooltip tip={labels.strategicDesireTip} />
                      </label>
                      <select
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Complexidade (1-5)
                        <Tooltip tip={labels.complexityTip} />
                      </label>
                      <input
                        type="number"
                        min="1"
                  ...
                  <div className="mb-5 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="col-span-2">
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Gatilho de Compra
                        <Tooltip tip={labels.purchaseTriggerTip} />
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Tempo Decisão
                        <Tooltip tip={labels.decisionTimeTip} />
                      </label>
                      <select
