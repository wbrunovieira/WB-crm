...
export function PartnerICPSection({ partnerId }: PartnerICPSectionProps) {
  const { data: linkedICPs = [], isLoading: loading } = usePartnerICPs(partnerId);
  const { data: availableICPs = [] } = useICPs("active");
  const linkMutation = useLinkPartnerToICP();
  const updateMutation = useUpdatePartnerICP();
  const unlinkMutation = useUnlinkPartnerFromICP();
  ...
  const handleUnlink = async (icpId: string) => {
    ...
    setUnlinking(icpId);
    try {
      await unlinkMutation.mutateAsync({ partnerId, icpId });
    } catch (err) {
  ...
  const handleExpand = (link: ICPPartnerLink) => {
    ...
    <div className="rounded-lg bg-white p-6 shadow">
      ...
      {linkedICPs.length === 0 ? (
        <p className="text-base text-gray-500">
          Nenhum ICP vinculado a este parceiro.
                ...
                <div className="border-t border-gray-100 bg-gray-50 p-5">
                  ...
                  <div className="mb-5 grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Observações
                        <Tooltip tip="Notas adicionais sobre o parceiro" />
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="mb-5 grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Status de Fit
                        <Tooltip tip="Quão bem o parceiro se encaixa no perfil ideal" />
                      </label>
                      <select
                  ...
                  <div className="mb-5">
                    <label className="mb-2 flex items-center text-sm font-semibold text-gray-700">
                      Momento do Negócio
                      <Tooltip tip="Em qual fase do negócio o parceiro está" />
                    </label>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mb-5">
                    <label className="mb-2 flex items-center text-sm font-semibold text-gray-700">
                      Plataformas Atuais
                      <Tooltip tip="Quais plataformas o parceiro usa atualmente" />
                    </label>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mb-5 grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      <label className="mb-1.5 flex items-center text-sm font-semibold text-gray-700">
                        Desejo Estratégico
                        <Tooltip tip="O que o parceiro deseja alcançar" />
                      </label>
                      <select
