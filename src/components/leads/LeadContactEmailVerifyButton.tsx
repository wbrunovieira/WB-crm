...
export function LeadContactEmailVerifyButton({
  ...
  leadContactId,
  verified,
}: LeadContactEmailVerifyButtonProps) {
  ...
  async function handleVerify() {
    ...
    try {
      const result = await apiFetch<{ ok: boolean; leadContactId: string; email: string; valid: boolean; status: string; reason: string }>(
        `/email/verify/lead-contact/${leadContactId}`,
        token,
        { method: "POST" },
      );
